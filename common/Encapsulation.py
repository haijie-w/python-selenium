...
class Assert():
    ...
    def assertStrategyExist(cls):
        ...
        str(cls.driver.find_element_by_class_name('list').text).split('：')[2].split('获')[0].split('\n')[0]).find(
                u'暂无数据') == -1:
            cls.logger.mylogger(
                    str('Token=' + cls.driver.find_element_by_class_name('list').text).split('：')[2].split('获')[
                        0].split('\n')[0])
        else:
...
class WebHandle():
    ...
    def elementIsNotVisible(cls, page,element):
        try:
            el = WebDriverWait(UIHandle.driver, 10).until_not(EC.presence_of_element_located(local_config[page][element]))
    ...
    def byIdAndXpath(cls, page, elementID, elementXpath):
        sub_element = cls.driver.find_element_by_id(local_config[page][elementID][1]).find_element_by_xpath(page, elementXpath)
        ActionChains(cls).click(sub_element).perform()

    def byIdAndText(cls, page, elementID, elementText):
        sub_element = cls.driver.find_element_by_id(local_config[page][elementID][1]).find_element_by_link_text(page, elementText)
        ActionChains(cls).click(sub_element).perform()
    ...
    def infoMatching(cls, filename, cellList, nrow, ncolumn, infoNum):
        ...
        for i in range(1, infoNum):
            ...
            str2 = str(cls.getText("/html/body/div[2]/div[1]/div[3]/div/div/div[2]/div/div[2]/div[1]/div[1]/div/div[2]/ul/li[" + str(j) + "]/h2"))  # 网页信息
    ...
    def loctorFromFather(cls, page, fa,sub,msg):
        el = cls.driver.find_element_by_xpath(local_config[page][fa][1]).find_element_by_xpath(local_config[page][sub][1])
        #清空输入框的内容
        el.clear()
        #输入msg
        el.send_keys(msg)
            ...
            if i==row:
                ...
                for j in range(collist):
                    if column==j:
                        ...
                        #如果只有一行path
                        if len(row_list)==1:
                            xpath='//*[@id="editTaxsModal3"]/div/div/div[2]/div[1]/div[2]/table/tbody/tr/td['+str(j)+']/div/input'
                        else:
                            #多行的xpath
                            xpath = '//*[@id="editTaxsModal3"]/div/div/div[2]/div[1]/div[2]/table/tbody/tr[' + x + ']/td[' + str( j) + ']/div/input'
    ...
    def InputAccountByDiv(cls, queryContent, page, element, account):
        ...
        for i in range(len(table_tr_list)):
            ...
            if queryContent in table_text:
                ...
                cls.driver.find_element_by_xpath('//*[@id="newProduct"]/div[1]/div[2]/div/div['+str(i+1)+']/div').find_element_by_id('InitAmount').clear()
                cls.driver.find_element_by_xpath('//*[@id="newProduct"]/div[1]/div[2]/div/div['+str(i+1)+']/div').find_element_by_id('InitAmount').send_keys(account)
                Logger.mylogger(queryContent + '存在！')
                falg = True
                break
        if not falg:
