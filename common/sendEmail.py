...
class SendEmail:
    def __init__(self):
        ...
        self.url = ReadConfig()
        # 设置服务器
        self.mail_host = self.url.get_email("HOST_SERVER")
        # 用户名
        self.mail_user = self.url.get_email("user")
        # 口令(qq邮箱非密码)
        self.mail_pass = self.url.get_email("password")
        # 发送邮件账号
        self.sender = self.url.get_email("FROM")
        # 接收邮箱，可设置为你的QQ邮箱或者其他邮箱
        self.receivers = self.url.get_email("TO")
        # 邮件标题
        self.subject = self.url.get_email("SUBJECT")
